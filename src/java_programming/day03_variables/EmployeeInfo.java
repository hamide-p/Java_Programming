package java_programming.day03_variables;

public class EmployeeInfo {

    public static void main(String[] args) {


       String employeeName = "Emily";
        int age = 30;
        char gender = 'F';
        String companyName = "google Inc",    // using for more than one string you can put comma
                employeeId = "A19",
                jobTitle = "Java Developer";
        int salary = 11000;
        boolean isFullTime = true;



        System.out.println(employeeName);
        System.out.println(age);
        System.out.println(gender);
        System.out.println(companyName);
        System.out.println(employeeId);
        System.out.println(jobTitle);
        System.out.println(salary);
        System.out.println(isFullTime);

        System.out.println("........................");

        System.out.println("employeeName = " + employeeName); // shortcut for variable is soutv + enter
        System.out.println("age = " + age);
        System.out.println("gender = " + gender);
        System.out.println("companyName = " + companyName);
        System.out.println("employeeId = " + employeeId);
        System.out.println("jobTitle = " + jobTitle);
        System.out.println("salary = " + salary);
        System.out.println("isFullTime = " + isFullTime);

        

        /*
        System.out.println("Aaron");
        System.out.println("Aaron");
        System.out.println("Aaron");

        System.out.println(".......................");

        String name = "Aaron";
        System.out.println(name);
        System.out.println(name);
        System.out.println(name);
         */




    }

}



/*
1. CreateaclassnamedEmployeeInfo.java
2. Declarethefollowingvariableswithappropriatedatatypes:
1. name
2. age
3. gender
4. companyName 5. employeeId
6. jobTitle
7. salary
8. isFullTime

 */